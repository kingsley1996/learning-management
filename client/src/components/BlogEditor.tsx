...
export default function BlogEditor({ value, onChange }: BlogEditorProps) {
  useEffect(() => {
    if (typeof window !== 'undefined') {
      hljs.configure({
        languages: ['javascript', 'typescript', 'css', 'html', 'python', 'java', 'json', 'bash'],
  ...
  return (
    <div className="bg-white min-h-[400px] rounded-xl shadow-lg border border-gray-200 overflow-hidden">
      <ReactQuill
        ...
        onChange={onChange}
        modules={modules}
        formats={['header', 'bold', 'italic', 'underline', 'strike', 'blockquote', 'list', 'link', 'image', 'video', 'color', 'background', 'code-block']}
      ...
      <style jsx global>{`
        ...
        /* Middle code blocks */
        .ql-editor .ql-code-block-container .ql-code-block:not(:first-child):not(:last-child) {
          border-radius: 0;
          border-top: none;
